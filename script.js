...
function isAutoGeneratedStream(title) {
  const lowerTitle = (title || "").toLowerCase();
  for (const pattern of AUTO_GENERATED_PATTERNS) {
    if (lowerTitle.startsWith(pattern.toLowerCase())) return true;
  }
  return false;
}
  ...
  try {
    ...
    const items = data.items
      ...
      .map(item => {
        ...
        // Filter out private, deleted, restricted, and auto-generated stream videos
        if (title === "Private video" || title === "Deleted video" || isAutoGeneratedStream(title)) {
          return null;
        }
